...
- Replaced `packages/server/src/server-state.ts` with lifecycle modules for public orchestration, types, snapshots, reloads, recovery, and probing.
...
- Largest new production file: `packages/server/src/server-state/index.ts` at 249 lines.
...
- `rtk bun run --filter @aio-proxy/server test:unit` — 378 passed, 0 failed, 1,180 assertions across 54 files.
...
- The required line-count command passed; the production split ranges from 6 to 249 lines per file.
...
- Moved reload transaction staging, compensation, finalization, and failure mapping out of `server-state/index.ts` into the concrete private `server-state/reload.ts` collaborator.
...
- Reduced `server-state/index.ts` to 249 lines while keeping orchestration and public exports there; `snapshot.ts` now contains only snapshot/build concerns at 215 lines.
...
- Made `rawCapability` private to `plugin-runtime/capabilities.ts` again.
- Reworked both array-boundary regressions to exercise `materializePluginProvider` through the public `plugin-runtime` entry API; no test imports the private capabilities module.
- Split reload behavior from snapshot construction into `server-state/reload.ts` and updated the authoritative plan and generated Task 8 brief so the documented module list matches the implementation.
