...
- Runtime coverage remained in `_test/runtime.test.ts`; its 5 raw/model runtime tests passed unchanged.
...
- `rtk bun run check`: PASS — 603 files checked; 0 errors, with 3 pre-existing warnings and 61 pre-existing informational diagnostics outside Task 9.
...
- `src/github-api/login.test.ts`: 263
- `src/plugin.test.ts`: 149
- `src/plugin.ts`: 146
- `src/github-api/login.ts`: 139
- `src/github-api/catalog.test.ts`: 83
- `src/github-api/credential.test.ts`: 65

The complete `src` tree is 1,238 lines across 16 TypeScript files. The genuinely shared package-local test support is 30 lines.

## Diff Audit

Command: `rtk proxy git diff HEAD --stat -- packages/plugins/github-copilot`

- 19 files changed, 1,078 insertions, 978 deletions.
...
- Added `_test/test-support.ts` for the fetch mock and adapter fixture shared by five test files; `_test/runtime.test.ts` now reuses it.
- Modified `src/index.ts` and `tsconfig.json` beyond those replacements.
- No Task 4 file is included in the Task 9 diff.

## Self-review

- Standards review found duplicated fetch-mock setup across five files; it was replaced with the 30-line package-local `_test/test-support.ts` used by all five callers.
- Spec review found the TTL constant defined in `plugin.ts` and two cases outside their original describe-block concerns. The public TTL constant now lives in `src/index.ts`; the invalid Enterprise case is in `login.test.ts`, and the TTL case is in `catalog.test.ts`.
