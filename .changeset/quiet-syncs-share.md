...
Add selective configuration sync, including required plugin settings, configuration history, and per-device local overrides. Every provider, model rule and plugin in your configuration is listed for selection and joins together with the plugin settings it needs. Synced OAuth accounts use coordinated refresh, remain pending on other devices until their adapter supports verified multi-device use, and must be detached before you disconnect or switch backends so no device is left rotating a credential the others still follow; detaching asks you to sign in again and keeps that login as this device's own credential. Plugin authors can register compatible sync backends through the SDK; the iCloud backend is still in verification and is not published with this release.
Remote imports retry safely after interruptions while preserving newer local configuration and plugin-secret edits, and a connect whose review is cut short — by a configuration change while the dialog is open, or by the service stopping partway through applying it — asks you to review again instead of importing objects you never saw; when the space already holds a provider, model rule or plugin you have configured locally, the review shows both sides so you choose which one wins, and for a model rule or plugin setting, which no rename can separate, it also offers to retire the duplicate copy the space holds. The sync commands refuse to send credentials to a remote endpoint over plain HTTP, and an incoming provider that would send one of this device's `{{env.NAME}}` secrets somewhere you have not already approved stays pending for review.
