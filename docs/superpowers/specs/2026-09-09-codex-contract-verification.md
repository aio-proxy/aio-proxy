...
A disposable probe ran the explicit executable `codex-cli 0.146.0` on `darwin/arm64`. It created a temporary home, a helper under a path containing spaces, and a loopback upstream. The helper was invoked through an argument array with empty stdin; raw token, JSON, empty output, non-zero exit, and over-5-second timeout inputs were exercised. Only token equality and sanitized failure descriptions were observed. The probe never read or wrote the real Codex files. The one-off probe is not retained as a repository script.
...
```json
{
  ...
  "refreshInvocationObserved": true,
...
The affected Codex configuration, command-auth, lifecycle, and session suites were run against the reviewed Task 6 implementation. A serial Codex run completed 96 passing and 0 failing tests across 11 files; the command-auth, setup, lifecycle, and session suites also passed when run independently. A parallel invocation of the Codex and broader CLI suites contended for shared ports and temporary credentials: it reported the existing run-port-conflict timeout, two refresh failures, and one endpoint-change recovery mismatch. Those parallel-only observations are recorded as environment evidence and are not counted as passing.

`bun run i18n:compile` completed successfully without changing generated i18n output. `bun run --filter @aio-proxy/cli build:binary` completed and produced darwin-arm64, darwin-x64, linux-arm64, and linux-x64 artifacts. The darwin-arm64 artifact reports `unknown command 'app-server'`; artifact build is therefore verified but native app-server compatibility is unverified. The host executable experiment remains the only passing native command-auth result. Its refresh-interval result records only an extra helper invocation; it does not establish proactive token refresh. Official Computer Use and plugin compatibility were not exercised and remain unverified.
