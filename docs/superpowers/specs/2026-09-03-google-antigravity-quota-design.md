...
Try `https://daily-cloudcode-pa.googleapis.com`, then
`https://daily-cloudcode-pa.sandbox.googleapis.com`, then
`https://cloudcode-pa.googleapis.com`, in that order — this is `ANTIGRAVITY_QUOTA_URLS` verbatim,
and daily is where a live Antigravity client actually gets answered.

The list goes into `antigravityEndpoints(options, 'quota')`
(`packages/plugins/google-antigravity/src/runtime/endpoints.ts`) as a new operation, so the existing
...
Each quota attempt gets `QUOTA_ENDPOINT_TIMEOUT_MS = 10_000`, so three bases cost at most 30s.
