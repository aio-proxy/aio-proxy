...
- 现有 API key `$NAME` 语法保持兼容；`{{env.NAME}}` 也可用于 `apiKey` 和其他字符串值。
...
Authoring 和 mutation schema 必须允许原本为字符串的字段携带 `{{env.NAME}}`，但不能因此放宽 materialized runtime schema。Dashboard mutation 仍先形成 raw candidate，再通过统一 runtime parse wrapper 展开并验证；验证失败时原子 transaction 不落盘。数字、布尔值和对象不能通过模板注入，因为本轮模板只替换字符串叶子。
