...
宿主对 import、setup、catalog 与 `createRuntime` 设置有界 deadline。无法中止的动态 import 或 local-only runtime promise 超时后只丢弃晚到结果；staging registry 会被封存，迟到的注册不能进入 snapshot。
...
若用户手工把 built-in identity 写进 `plugins`，loader 将其视为同一个 embedded plugin，而不是第二个来源；列表只显示一次，cache 仍被忽略。该 entry 的 options 仍按 embedded descriptor 校验，无 options 的 built-in 收到非空 options 时失败为 `PLUGIN_OPTIONS_INVALID`。

### Capability

插件通过 setup 注册的能力。capability ID 必须匹配小写稳定 token `/^[a-z0-9][a-z0-9._-]*$/`，并在当前插件内唯一；宿主用 `{ plugin, capability }` 组成无冲突引用。v1 只有 OAuth capability，但接口不假设一个插件永远只能注册一个能力。
...
6. 从 plugin-scoped vault 合并已有 secret，并执行插件提供的 Zod schema 校验。
...
同一 server 进程中的显式 reload 与 config-watcher reload 必须串行；较早的慢候选不能在较新的配置之后完成并覆盖新 snapshot。
...
请求通过 reference-counted snapshot lease 持有旧 runtime。显式 provider 删除对新请求立即生效，但账号 credential/catalog 的物理删除等旧 snapshot lease drain 后执行；进程崩溃时由 delete marker recovery 完成。
...
`@aio-proxy/plugin-sdk` 是小型、独立发布的包。它直接依赖 Zod 与 `@ai-sdk/provider`，并从公共入口以 `export { z as zod } from "zod"` 导出自己的 Zod runtime value 与相关类型；插件必须从 `@aio-proxy/plugin-sdk` 导入 `zod`，不再直接依赖或导入 `zod` package。SDK 不能依赖 `@aio-proxy/core`、server、CLI 或数据库实现。

宿主通过 schema 的 `safeParseAsync()` 执行同步或异步校验，并把 Zod issues 归一化为不含输入值的安全错误。loader 不用 `instanceof` 判断 schema：第三方包位于独立 npm cache，可能加载兼容 API version 的另一份 SDK/Zod module；公共契约由 SDK 导出的类型、必需方法与 descriptor `apiVersion` 共同约束。
...
Zod 是 SDK runtime contract 的一部分。插件 schema 只从 SDK 导出的 `zod` 创建；未来若升级到不兼容的 Zod major，必须与新的 descriptor API major 一起发布，不能在 `apiVersion: 1` 内静默改变 schema 语义。
...
字段可以包含 label、description、placeholder、静态 select options，以及一个简单的同级字段等值显示条件。复杂的数据正确性只由 Zod schema 决定；form 不建立第二套验证规则。
...
adapter 的概念形状：
    ...
    type OAuthAdapter<AccountOptions, Credential> = {
      ...
      credentials: ZodType<Credential>
...
- 新账号先规范化 suggestedKey。若该 key 已被其他账号占用，宿主追加 `{ plugin, capability, fingerprint }` 的稳定 hash 前缀，并在极小概率仍冲突时逐步延长 hash，直到得到唯一 ID。相同输入始终得到相同候选 ID，不使用依赖当前顺序的 `-2` 计数。
- 不带 `--provider` 的新增流程若发现相同 namespaced fingerprint 已存在，会停止提交并提示对现有 Provider ID 执行显式 re-login；它既不创建第二个 provider，也不静默覆盖旧账号。显式 re-login 必须返回与目标账号相同的 fingerprint。
...
- callback URI、state 或 code 无效时返回安全错误并继续等待，不能抢先终止合法会话；匹配 state 的标准 OAuth denial/error 才结束为失败。
...
- 宿主创建 AuthorizationPort 时把它绑定到同一个登录 deadline/AbortSignal；adapter 取消或 20 分钟登录超时必须同时终止 loopback/manual input。
- server runtime 不得自动启动交互式登录，只返回修复建议。

## 登录与持久化事务

所有 CLI 与 server/Dashboard 配置写入共享同一个跨进程文件锁，并在锁内完成 read-modify-rename；server 写入还在锁内等待候选 snapshot 验证，失败时恢复原始字节。文件锁避免两个配置写进程互相覆盖，pending marker 则负责 config 与 SQLite 之间无法合并的事务边界。

`provider login [capability]` 默认创建新账号。`--provider <id>` 显式进入 re-login：宿主要求该 Provider ID 同时存在结构化 config entry 与账号 vault row，从 entry 推导 plugin/capability，并预载公开 account options 与账号级 secret；若同时给了 capability 参数，两者必须一致。entry 已缺失的 orphan/delete-pending 账号不在 CLI 中复活，命令返回 cleanup-pending，等待 server 完成安全删除后再按新增流程登录。这样 secret 可以在 OAuth 开始前合并，而不是等 fingerprint 返回后猜测目标账号。

宿主流程固定如下：

1. 解析注册完成的 `{ plugin, capability }`；re-login 同时读取并交叉校验目标 provider entry/account 的初始 runtime revision、fingerprint、公开 options 与 secret。
2. 通过账号级 ConfigSpec 收集普通值与 secret；只有 re-login 具有“空 secret 保留旧值”语义，新增流程没有可继承 secret。
3. 合并值并执行 account options schema。
4. 调用 adapter login；此阶段所有输入与 credential 只驻留内存。
5. 用 adapter credential schema 验证返回值。
6. 使用内存中的新 credential 尝试首次 model discovery；失败不回滚登录。re-login 有 last-known-good 时继续保留旧目录并标记 stale，从未有可用目录时 provider 标记为 `unavailable`。
7. 完成所有远程工作后获取配置文件锁，重新读取 raw config 与账号 runtime revision。
8. 新增流程若发现 namespaced fingerprint 已存在且对应 entry 仍存在，则停止提交并提示 `provider login --provider <existing-id>`；若仅有 orphan/delete-pending account，则返回 cleanup-pending。re-login 要求目标 entry、account runtime revision 与 fingerprint 仍匹配，并要求 adapter 返回同一 fingerprint。普通 token refresh 只改变 credential revision，不应使 re-login 无故失败；re-login 提交后，旧 refresh 的 credential CAS 必须失败。
9. 新账号由宿主根据 suggestedKey 生成最终 ID；re-login 固定复用目标 Provider ID，并从锁内最新 entry 保留 routing 字段。
...
步骤 7–11 中所有依赖当前 config 的判断（fingerprint 查重、Provider ID 冲突、目标 runtime revision、re-login routing 字段）都在网络登录完成后、持有配置文件锁时重新读取并最终决定；远程 OAuth 与 discovery 不得占用文件锁。

配置与 SQLite 无法组成单个底层事务，因此提交顺序保证 config 不会指向尚未写入的 credential。崩溃恢复先处理 pending marker，再清理超过 30 分钟、无 marker 且 Provider ID 已不在原始 config record 中的账号。plugin remove 不会触发该清理，因为它保留 provider entry；30 分钟窗口长于登录上限，避免与活跃登录竞争。

显式 provider 删除使用同一机制的 `delete` marker：先记录目标账号 runtime revision，再移除 config；旧 snapshot drain 后重新获取配置文件锁，只有 raw provider entry 仍不存在且 runtime revision 未变化时才删除账号及级联数据。普通 credential refresh 不改变 runtime revision，因此不会无故阻塞删除；重新加入 config、重新创建或 re-login 的新数据不得被旧 marker 删除。
...
运行中的 server 根据最早 marker TTL 或 orphan grace deadline 设置一个宿主 recovery timer；处理后重新计算下一次时间，server close 时清理。恢复不依赖 plugin lifecycle hook。

CLI 只补偿 create/update marker；delete marker 与 orphan 物理清理由 server 持有 snapshot-drain 语义后执行，避免另一个进程仍在使用旧 credential 时提前删除。server recovery 接收一个按 Provider ID 判断删除是否安全的宿主 predicate；只要 current/retired snapshot 仍持有该 provider，就保留 marker/账号并安排下一次恢复。所有 delete/orphan 的“config 中仍不存在”判断都在共享配置文件锁内完成，不能用锁外的过期读取决定物理删除。

re-login 是更新事务：

- 只有显式 `--provider <id>` 才进入 re-login；
- adapter 返回的 fingerprint 必须与目标账号一致，否则不修改旧账号；
...
- 新增流程遇到同一 fingerprint 不创建第二个 provider，并提示显式 re-login；
...
- revision 未变化时，lease owner 才调用一次 exchange，校验返回 credential schema，并用内部 revision CAS 提交。宿主向 exchange 传入 deadline `AbortSignal`，adapter 的网络请求必须使用该 signal。其他进程等待 lease 释放或过期，然后重新读取；它们不得并行消费同一个 rotating refresh token。
...
v1 现有文本协议路由与 `/v1/models` 只消费 `catalog.language`；其他 modality 会被校验、缓存并传回 adapter runtime context，但暂不进入文本路由。
...
插件返回或数据库读出的 catalog 在使用前必须经过宿主运行时校验：六个 modality 数组齐全、ID 非空、同一 modality 内不重复，metadata 只能是有限数字与无环 JSON value。诊断只包含 modality/index/path，不回显 metadata。
...
- 失败后由宿主按 5 分钟 retry window 重新调度，snapshot rebuild 本身不得触发即时重试循环。
...
宿主在 snapshot 构建时验证 `provider.specificationVersion === "v4"`、全部 required 方法可调用，以及已提供的 optional 字段形状正确。validator 必须接受普通对象和 callable function object，因为 `createOpenAI()` 等官方 provider 本身可调用但仍完整实现 ProviderV4。`files()` 与 `skills()` 虽属于完整 ProviderV4 seam，但超出 v1 catalog/routing 范围。`videoModel` 不属于本项目锁定版本的 ProviderV4，不能作为兼容性要求。
...
账号持久化区分 credential revision 与 runtime revision：refresh 只增加前者，re-login/account options 变化增加后者。诊断-only snapshot 使用 runtime identity key 复用旧 runtime；plugin/options/runtime revision/catalog refresh 变化才重新调用 `createRuntime`。
...
状态保持为两个小型 union：
    ...
    type ProviderState =
      | { status: "ready"; catalog?: "fresh" | "stale"; diagnostic?: Diagnostic }
      | { status: "unavailable"; diagnostic: Diagnostic }

OAuth provider 的 ready state 必须带 `catalog`；API/AI SDK provider 的 ready state 省略该字段。这样 Dashboard 可以共用一个小型状态 union，而不为没有宿主 catalog 的 provider 伪造 freshness。
...
`Diagnostic` 包含稳定 code、安全摘要、是否可重试、发生时间和建议命令。Dashboard 只展示这些安全字段；原始 cause 与 stack 只写本地日志，并经过 credential、secret、authorization code 与 URL query redaction。宿主把当前 ConfigSpec secret 值与 credential 中的字符串叶子作为动态 redaction values，不能只依赖固定字段名。

持久化诊断按 `{ providerId, code }` 共存，避免 catalog failure 覆盖 credential failure；ProviderState 展示时使用 credential > catalog 的稳定优先级，并可在 ready/stale 状态附带 catalog diagnostic。
...
- `plugin prune`
- `provider login`
- 现有 CLI provider list/test 入口与 Dashboard provider delete 入口
...
`provider login [capability]` 解析已注册 OAuth capability；无歧义的短 capability ID 可以直接使用，发生歧义时要求 canonical `plugin + capability` 引用或交互选择。`--provider <id>` 表示显式 re-login，可省略 capability 并从现有 entry 推导；若两者都提供则必须匹配。最终配置始终保存结构化引用，不保存 CLI alias。
...
依赖方向固定为单向文字规则：built-in/third-party plugin 只依赖 plugin SDK；core plugin host 也只依赖 plugin SDK 的公共契约；CLI 与 server 再依赖 core host。plugin SDK 不反向依赖 core，built-in plugin 也不从 server 或 CLI 导入实现。
...
3. 将 GitHub Copilot 复制、适配为 built-in plugin，并通过现有行为 fixture；旧源码在切换前保留。
4. 将 OpenAI ChatGPT 复制、适配为 built-in plugin，并通过 loopback/PKCE fixture；旧源码在切换前保留。
...
- 新增流程 fingerprint 重复时不写入并提示显式 `--provider`；
- 显式 re-login 在 OAuth 前复用旧 options/secret，固定复用 Provider ID，并拒绝 fingerprint 不匹配；
...
- 删除 provider 在 snapshot drain 后、配置锁内重新确认 entry 缺失并按 runtime revision 级联清理；credential refresh 不阻塞删除，重新加入/re-login 会使旧 marker 失效。
