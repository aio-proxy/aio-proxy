...
plugin warning，并继续提交 capability。它不产生 failed plugin state，也不使 provider unavailable。
...
Credential refresh 继续通过现有 `CredentialPort` lease + CAS 契约完成。Quota capability 不获得
credential 原始存储或绕过该 port 的权限。
