...
- `onValueChange` hands back `number | readonly number[]` for the same reason, so forwarding it straight into `onChange: (value: number | undefined) => void` is a `TS2322` — the one half the compiler DOES catch.
`packages/ui/src/components/slider.tsx` is registry output and `packages/ui/src/components/AGENTS.md` forbids editing it, so this is adapted at the call site, not fixed upstream. Cover the thumb count in Step 1: one thumb for a stored weight, one for an absent weight.
