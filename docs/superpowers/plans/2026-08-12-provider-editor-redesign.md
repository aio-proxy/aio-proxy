...
- Unchanged, reused as-is: `provider-alias-list.tsx`, `provider-alias-card.tsx`, `provider-alias-draft.tsx`, `provider-alias-config-fields.tsx`, `provider-alias-variants.tsx`, `provider-alias-delete-dialog.tsx`, and **`use-alias-drafts.ts`** — see the draft-layer note below
...
- Modify: `packages/i18n/messages/*.json` — retire `dashboard.providers.editor.step_*`, `editor.next`, `editor.previous`, `dashboard.providers.oauth.models_readonly`; reword `form.metadata_description`, `form.metadata_json_label`, `form.metadata_json_error` to stop asserting the editor is JSON-only (all five locales; parity test keeps this honest); run `bun run i18n:compile`
...
- [ ] **Step 4: Retire/reword the i18n keys listed above in all five locales, run `bun run i18n:compile`, and confirm `rg -n "step_connection|step_models|step_routing|step_validate|step_invalid|models_readonly" packages/dashboard/src packages/i18n/messages` only matches the reworded metadata keys.**
