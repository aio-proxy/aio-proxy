...
- **`test:unit` cannot see type errors** — rstest strips types with SWC, it does not typecheck. So every task from 13 on, after its `test:unit` passes, also runs:
  ...
  Expected: exit 1 with **at most 12 errors** — the 9 inherited from `main` (`providers-table-columns.tsx` ×5, `provider-request-transforms-visual-editor.tsx` ×2, `provider-validate-step.tsx`, `stage-codec.ts`) plus **3 that task 14 introduces on purpose** in the two legacy templates task 19 deletes: `provider-form-page.tsx:150` and `:160` (`TS2322: Type 'ProviderForm' is not assignable to type 'ProviderEditorForm'`) and `oauth-provider-edit-page.tsx:67` (`TS2322` against the shrunken `OAuthProviderEditFieldsProps`). **Do not fix those three** — task 14's own step text says so, and the only honest repair is task 19's deletion. Any error in a file *your* task created or modified is yours to fix before committing. Counted after task 14: 12 through task 18, then 8 once task 19 deletes both templates and `provider-validate-step/`. (Task 14 also cleared what used to be a tenth inherited error, `provider-form-fields-ai-sdk.tsx:56`'s `useRef<string>()`, deleted with the dead ref it typed.) Without this line a cross-task type break (task 13's form union reaching task 15's hook, task 17's `ProviderEditorForm` params) stays invisible for six more tasks and lands on the wrong implementer.
- **Driving a Base UI `Select` in a test: use `keyDown` `Enter`, never a plain `click`.** Measured under jsdom in task 14: `fireEvent.click` on an option commits only the item that is *already* selected, so clicking any other option fires no `onValueChange` and the spy reports `Number of calls: 0`. `fireEvent.keyDown(option, { key: 'Enter' })` commits any option; `pointerDown`+`pointerUp` commits none. This is a silent false-green in both directions, so it matters for every remaining Select assertion: task 17's proxy tri-state (`inherit`/`disabled`/`url`), task 15's metadata controls, task 16's alias rows. Do not copy the pattern at `provider-form-fields-api.test.tsx:19-25` as a model for *value* assertions — it clicks `OpenAIResponse`, which is the first `ProviderProtocol` member and therefore the already-selected item, so it passes for a reason that does not generalize (its real purpose is the option/trigger icon, which is fine). Working example: `provider-editor/identity-section/identity-section.test.tsx`, the kind-picker case.
...
- [ ] **Step 3: Run to verify pass, then commit.** Run: `bun run --filter @aio-proxy/dashboard test:unit` — PASS, and the old `provider-validate-step.test.tsx` still passes too (both exist until task 19). Then the Global Constraints type-aware lint: still exactly the 12 recorded errors, no 13th.
