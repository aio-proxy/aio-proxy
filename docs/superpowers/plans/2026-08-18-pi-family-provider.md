...
- Official Pi owns one raw interval and clears it on `session_shutdown`. OMP uses `ctx.setInterval`, whose lifecycle is host-managed; do not create a raw OMP timer.
...
import { loginPiFamily, refreshPiFamilyCredential, toPiFamilyModels } from './core';
...
import type { AgentCatalogV1, AgentDeviceCodeResponse, AgentManagedMarker } from '@aio-proxy/types';
...
export async function loginPiFamily(
  ...
  if (catalog.error === 'unauthorized') throw new Error('aio-proxy login required');
...
export async function readPiFamilyModels(
  ...
  if (accessToken === undefined) {
    const lkg = await readLastKnownCatalog(managed.statePath, managed.marker.agent);
    return { models: lkg === null ? [] : toPiFamilyModels(lkg) };
  }
...
`core/index.ts` contains only explicit exports of the types/functions above. Do not export `CoreOptions`; it is inferred at call sites and exists only to keep unit effects deterministic.
...
test('catalog 401 keeps host LKG and returns a stable re-login diagnostic', async () => {
  const f = await fixture({ catalogResults: [{
    models: toPiFamilyModels(hostCatalog('lkg')), error: 'unauthorized',
  }] });
...
async function fixture(options: { readonly catalogResults?: PiFamilyCatalogResult[] } = {}) {
  ...
  const deps: OfficialPiDeps = {
    ...
    readPiFamilyModels: async (_managed, access) => {
      ...
      return catalogResults.shift() ?? { models: toPiFamilyModels(lkg) };
    },
  ...
  const refreshModels = createSingleFlight(async (context: RefreshModelsContext): Promise<ProviderModelConfig[]> => {
    ...
    const result = context.allowNetwork
      ...
      : await deps.readLastKnownCatalog(managed.statePath, 'pi').then((current) => ({
          models: current === null ? [] : toPiFamilyModels(current),
        }));
...
Expected: PASS; the host-refreshed credential is the only catalog access token, exactly one timer exists, and shutdown clears it.
...
- Consumes: OMP `ExtensionAPI`, `ExtensionContext`, `fetchDynamicModels(apiKey)`, and ModelRegistry APIs as type-only contracts plus Task 1 core.
- Produces: default OMP `ExtensionFactory`; one registered `aio-proxy` provider; one host-managed 5-minute refresh timer.
...
test('catalog 401 force-refreshes host auth once and retries only with the returned key', async () => {
  const f = await fixture({ catalogResults: [
    { models: fModels(), error: 'unauthorized' },
    { models: fModels('fresh') },
  ] });
...
test('a second catalog 401 requires login after exactly one forced refresh', async () => {
  const f = await fixture({ catalogResults: [
    { models: fModels('lkg'), error: 'unauthorized' },
    { models: fModels('lkg'), error: 'unauthorized' },
  ] });
  const getApiKeyForProvider = mock(async () => 'aio_agent_at_v1_new');
  await f.emit('session_start', {
...
test('OMP never makes an anonymous catalog request', async () => {
  const f = await fixture();
...
async function fixture(options: { readonly catalogResults?: PiFamilyCatalogResult[] } = {}) {
  let provider: ProviderConfig | undefined;
  const catalogAccesses: Array<string | undefined> = [];
  const results = [...(options.catalogResults ?? [{ models: fModels() }])];
  const handlers = new Map<string, (...args: unknown[]) => unknown>();
  const timers = new Map<number, () => void | Promise<void>>();
  let timerSequence = 0;
  const setInterval = mock((callback: () => void | Promise<void>) => {
    const id = ++timerSequence;
    timers.set(id, callback);
  ...
  const deps: OmpDeps = {
    ...
    readPiFamilyModels: async (_managed, access) => {
      catalogAccesses.push(access);
      return results.shift() ?? { models: fModels() };
    },
  };
...
export async function registerOmp(pi: ExtensionAPI, deps: OmpDeps): Promise<void> {
  ...
  const fetchModels = createSingleFlight(async (apiKey: string | undefined): Promise<ProviderModelConfig[]> => {
    if (apiKey === undefined) throw loginRequired();
    const first = await deps.readPiFamilyModels(managed, apiKey);
    if (first.error !== 'unauthorized') return first.models;
...
Expected: PASS; 401 recovery makes at most one force-refresh and no anonymous request, no raw OMP timer is created, and both `dist/official-pi.js` and `dist/omp.js` exist.
...
- Produces: one repeatable `test:compat` command proving the correct manifest entry, native login callback, host credential persistence, catalog projection, replay-safe concurrent refresh, and inference for each pinned host version.
...
async function runScenario(
  ...
  try {
    ...
    if (scenario === 'concurrent') {
      ...
      assertSuccessfulPair(results, proxy.stats(), label);
      return;
    }
    ...
    check(/login|auth|credential|invalid_grant/iu.test(`${result.stdout}\n${result.stderr}`),
...
The OMP storage branch must create its normal `<agentDir>/agent.db` through `discoverAuthStorage(agentDir)`; it must never fabricate OMP `auth.json`. The crash scenario deliberately discards one returned rotation and flips the fake server directly to post-replay `invalid_grant`, so Task 3 of the control-plane plan remains the single test of the exact 30-second clock boundary. `PI_OFFICIAL_COMPAT_VERSIONS` and `OMP_COMPAT_VERSIONS` accept comma-separated floor/current lists; their defaults are the pinned floors above.

- [ ] **Step 4: Run real-host compatibility GREEN**

Expected: PASS for official Pi `0.84.2` and OMP `17.3.7`; each real loader selects only its native entry, the official Pi loader/probe executes under Node (not Bun), concurrent processes share one rotation, and crash-window loss fails closed with a login diagnostic.

Expected: PASS for official Pi `0.84.2` and OMP `17.3.7`; each real loader selects only its native entry, concurrent processes share one rotation, and crash-window loss fails closed with a login diagnostic.
