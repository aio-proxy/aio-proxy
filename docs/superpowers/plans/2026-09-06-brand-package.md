...
- One patch changeset, targeting `aio-proxy` alongside `@aio-proxy/brand`, `@aio-proxy/dashboard`, and `@aio-proxy/ui`. An earlier revision of this plan said "no changeset," calling this pure internal refactoring; a PR review caught that as false. The wordmark stops depending on a webfont and changes width, and the favicon's dark-mode ink changes from pure white to `#fbfbf9` — both user-visible. Per `CLAUDE.md` the changeset must include `aio-proxy` or the note never reaches a published Release.
