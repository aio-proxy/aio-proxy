...
- Object shape checks:
  ...
  - Do **not** use `isPlainObject` for structural TypeScript contracts that may be class instances (`PluginDescriptor`, `ConfigSpec`, `OAuthAdapter`, `OAuthLoginResult`, `ModelCatalog`, `LogicalRequestContext`, AI SDK language models, capability objects, `Error` subclasses). Inline `value !== null && typeof value === 'object' && !Array.isArray(value)`, or a file-local `isObject` with that body. Never name it `isRecord`. Never export it from `@aio-proxy/types` or plugin-sdk.
  - Do not invent a shared object-guard helper. Duplicate the three-line check where a structural contract needs it.
