...
// Nesting is capped at parse because nothing downstream survives an arbitrarily deep
// body: both onward transports hand it to recursive third-party code that overflows the
// call stack far sooner than `JSON.parse` does. Uncapped, that `RangeError` surfaces
// inside a candidate attempt as a 502 blaming a healthy upstream and cooling it down.
// These cases pin the boundary, and that the rejection is the protocol's own 400-mapped
// error rather than any stack overflow.
describe('parseSystemOneBody nesting depth', () => {
  // The body object is level 1, so N brackets under `deep` reach level N + 1 and 511 is
  // the deepest accepted body. Fixtures are built from repeated strings, not literals.
  const UNDER_CAP = 511;
  const OVER_CAP = UNDER_CAP + 1;
  const nest = (depth: number, leaf: string) => '['.repeat(depth) + leaf + ']'.repeat(depth);
  const withDeep = (depth: number, leaf = '0') =>
    post(`{"model":"m","state":"s","questions":{"q":{"type":"noul","instructions":"i"}},"deep":${nest(depth, leaf)}}`);

  it('accepts a body nested just under the cap', async () => {
    expect((await parseSystemOneBody(withDeep(UNDER_CAP))).model).toBe('m');
  });

  it('rejects a body nested just over the cap', async () => {
    await expect(parseSystemOneBody(withDeep(OVER_CAP))).rejects.toBeInstanceOf(SystemOneParseError);
    await expect(parseSystemOneBody(withDeep(OVER_CAP))).rejects.toThrow('nested more than 512 levels deep');
  });

  // The scan reaches the bottom of a legal body, so the depth cap did not shorten it:
  // the two guards coexist rather than one masking the other.
  it('still finds a non-finite number at the bottom of a body nested just under the cap', async () => {
    await expect(parseSystemOneBody(withDeep(UNDER_CAP, '1e400'))).rejects.toThrow(
      'Request body contains a non-finite number',
    );
  });

  // Kept from the fix that made this walk iterative: its point is that NO input depth
  // reaches a stack overflow. 50_000 levels is ~100 KB, far inside the encoded-body
  // limit, and deeper than any recursive walk in this process survives. The answer is
  // now a rejection rather than a parse, but it must still be the protocol's own error
  // -- a `RangeError` here would mean a depth got past the cap into a recursive walk,
  // and would reach the client as a 5xx or a 502 instead of a 400.
  it('rejects a body far past the cap with the depth error, never a stack overflow', async () => {
    const rejection = parseSystemOneBody(withDeep(50_000));
    await expect(rejection).rejects.toBeInstanceOf(SystemOneParseError);
    await expect(parseSystemOneBody(withDeep(50_000))).rejects.not.toBeInstanceOf(RangeError);
    await expect(parseSystemOneBody(withDeep(50_000))).rejects.toThrow('nested more than 512 levels deep');
  });
  ...
  // The cap is about depth, so it must not start rejecting breadth either.
