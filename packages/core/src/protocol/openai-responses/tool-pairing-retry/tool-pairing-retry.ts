...
// Narrates unpaired tool items as plain messages, in place, so the array order
// the caller sent is preserved. Mirrors the model path's semantics exactly
// (convertUnansweredToolCall / convertOrphanToolCallOutput in
...
export function repairOpenAIResponsesToolPairing(input: readonly unknown[]): unknown[] | undefined {
  ...
  const next = input.map((item) => {
    ...
    if (CALL_TYPES.has(type)) {
      ...
      if (callId !== undefined && answered.has(callId)) return item;
      changed = true;
      return unansweredCallNote(item, type);
    }
    if (!OUTPUT_TYPES.has(type)) return item;
    ...
    if (callId !== undefined && seen.has(callId) && answered.has(callId)) return item;
    changed = true;
    return orphanOutputNote(item, callId);
  });
