...
test('request transform changes rebuild an OAuth runtime while unchanged transforms reuse it', async () => {
  const fixture = runtimeFixture({ kind: 'static' });
  const base = {
    id: 'person',
    kind: ProviderKind.OAuth,
  ...
  const firstConfig = {
    ...base,
    transforms: { request: [{ update: [{ $set: { 'request.body.mode': 'one' } }] }] },
  } as const;
  ...
  const changed = await materializePluginProvider({
    ...
    config: {
      ...base,
      transforms: { request: [{ update: [{ $set: { 'request.body.mode': 'two' } }] }] },
    },
