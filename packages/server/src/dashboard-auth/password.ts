...
const ARGON2ID_PATTERN = /^\$argon2id\$v=19\$m=\d+,t=\d+,p=\d+\$[A-Za-z0-9+/]+={0,2}\$[A-Za-z0-9+/]+={0,2}$/u;
...
export async function normalizeDashboardPassword<T extends Record<string, unknown>>(config: T): Promise<T> {
  ...
  if (password.startsWith(ARGON2ID_PREFIX)) {
    if (!ARGON2ID_PATTERN.test(password)) throw new Error("Invalid Argon2id password hash");
    try {
