...
test.each([
  "read",
  "reset",
] as const)("a failed credential refresh during quota %s logs without adding a routing diagnostic or callback", async (operation) => {
  const fixture = createQuotaFixture(
    operation === "read" ? { read: failedRefresh } : { read: async () => availableQuotaSnapshot, reset: failedRefresh },
  );
...
test.each([
  "read",
  "reset",
] as const)("quota %s credential refresh redacts secrets derived from parsed account options", async (operation) => {
...
test.each([
  "read",
  "reset",
] as const)("a successful credential refresh during quota %s preserves an existing diagnostic and skips callbacks", async (operation) => {
