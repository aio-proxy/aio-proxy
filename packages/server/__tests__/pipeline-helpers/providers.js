...
import { aioHome, Router } from '@aio-proxy/core';
...
import { REQUESTED_MODEL } from './types';
...
const seededHomes = new Set();
function seedEmptyPriceCatalog() {
  const home = aioHome();
...
export function rawProvider(options) {
  ...
  const rawInvoke = async (request, context, transportOptions) => {
...
export function defineProviderRouteSource(fixtures, immediateStreamCompletion, debugLogging, routing = {}) {
...
function instrumentModel(model, calls) {
  return {
...
function languageCapabilityIndex(...modelIds) {
  return Object.fromEntries(modelIds.map((id) => [id, new Set(['language'])]));
}
function routeAlias(model) {
  return { [REQUESTED_MODEL]: { model, preserve: false } };
}
function routingFields(options) {
  return {
    ...(options.priority === undefined ? {} : { priority: options.priority }),
    ...(options.weight === undefined ? {} : { weight: options.weight }),
  };
}
export function withSnapshotConfigs(source, acquired, current = acquired) {
...
export function retryConfig(overrides = {}) {
