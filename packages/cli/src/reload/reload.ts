...
export async function reloadCommand(options: ReloadOptions = {}): Promise<void> {
  ...
  try {
    ...
    throw new ReloadError(
      m['cli.reload.failed']({ error: cause instanceof Error ? cause.message : String(cause) }),
      true,
    );
  }
  ...
  if (!response.ok || body.ok !== true) {
    ...
    throw new ReloadError(m['cli.reload.failed']({ error: body.error ?? `HTTP ${response.status}` }), transient);
  }
}
