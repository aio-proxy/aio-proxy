...
type SettlementController = {
  ...
  readonly losingPath: AbortController;
  readonly settle: (settlement: Settlement, afterResponse?: boolean) => boolean;
  readonly isSettled: () => boolean;
};

function createSettlement(): SettlementController {
  let settled = false;
  ...
  return { result, losingPath, settle, isSettled: () => settled };
}

function buildAuthorizationUrl(request: LoopbackRequest, expectedRedirectUri: string): string {
  ...
  return requireHttpUrl(builtAuthorizationUrl).href;
}

type ManualCallbackArgs = {
  ...
  readonly settle: (settlement: Settlement) => boolean;
};

async function pollManualCallback(args: ManualCallbackArgs): Promise<void> {
  const { deps, request, combinedSignal, authorizationUrl, expectedRedirectUri, settle } = args;
...
export async function runLoopbackAuthorization(
  ...
  try {
    ...
    if (request.allowManualCallbackUrl && process.stdin.isTTY === true) {
      void pollManualCallback({ deps, request, combinedSignal, authorizationUrl, expectedRedirectUri, settle });
    }
