...
describe('paraglide compile output', () => {
  ...
  test('emits an aggregated typed m object', () => {
    ...
    expect(typeof m['cli.run.description']).toBe('function');
    expect(typeof m['error.provider_not_installed']).toBe('function');
    expect(m['cli.error.port_out_of_range']({ port: 99_999 })).toContain('99999');
  });
});
