...
describe("paraglide tree-shaking spike", () => {
  test("documents aggregated m bundle retention for unused message names", async () => {
    ...
    writeFileSync(entry, 'import { m } from "./paraglide/messages.js"; console.log(m.spike_msg_001());\n');

    // When
    const build = Bun.spawnSync(["bun", "build", "--bundle", "--target=bun", "--minify", entry, "--outfile", bundle]);
