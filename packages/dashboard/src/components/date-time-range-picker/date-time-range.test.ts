...
describe('date time ranges', () => {
  ...
  test('turns invalid external endpoints into an empty draft', () => {
    ...
    ).toEqual({ from: '', to: '2026-07-20T23:59' });
  });
  ...
  test('formats a complete incoming value into an editable draft', () => {
    ...
    ).toEqual({ from: '2026-07-20T00:00', to: '2026-07-21T23:59' });
  });

  test('parses strict endpoint text for Calendar selection', () => {
    const pattern = "yyyy-MM-dd'T'HH:mm";
    expect(parseDateTimeEndpoint('2026-07-20T12:34', 'from', pattern, enUS)).toEqual(
      new Date(2026, 6, 20, 12, 34, 0, 0),
    );
    expect(parseDateTimeEndpoint('bad', 'to', pattern, enUS)).toBeUndefined();
    expect(parseDateTimeEndpoint('2026-7-20T12:34', 'from', pattern, enUS)).toBeUndefined();
  });
  ...
  test('fills omitted seconds with inclusive start and end boundaries', () => {
    ...
    const parsed = schema.parse({ from: '2026-07-20T12:34', to: '2026-07-20T13:45' });
  ...
  test('rejects malformed, reversed, and out-of-bounds drafts at their public paths', () => {
    ...
    const issuePaths = [
      schema.safeParse({ from: 'bad', to: '2026-07-20T12:00' }),
      schema.safeParse({ from: '2026-07-21T12:00', to: '2026-07-20T12:00' }),
      schema.safeParse({ from: '2026-06-30T23:59', to: '2026-07-20T12:00' }),
      schema.safeParse({ from: '2026-07-20T12:00', to: '2026-08-01T00:00' }),
...
testInNewYork('rejects gaps and expands repeated local end times', () => {
  ...
  expect(schema.safeParse({ from: '2026-03-08T02:30', to: '2026-03-08T03:30' }).success).toBe(false);
  const overlap = schema.parse({ from: '2026-11-01T01:30', to: '2026-11-01T01:30' });
