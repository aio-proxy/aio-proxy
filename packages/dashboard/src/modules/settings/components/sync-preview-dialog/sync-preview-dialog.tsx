...
export const SyncPreviewDialog: React.FC<SyncPreviewDialogProps> = ({
  ...
  const form = useForm({ defaultValues: initialValues(preview) });
  ...
  const apply = () => {
    ...
    const decisions = Object.values(form.getFieldValue('decisions'));
      ...
      <DialogContent className="max-h-[min(90vh,52rem)] max-w-4xl overflow-y-auto" closeLabel={m['common.close']()}>
        ...
        {preview === null || rows.length === 0 ? (
          ...
          <div className="space-y-4">
            ...
            {rows.some((row) => row.dependencies.length > 0) ? (
                    ...
                    {(pathField) => (
                      <div className="mt-3 flex gap-2">
                        ...
                        <Button
                          ...
                          onClick={() => {
                            const path = pathField.state.value
                              .split('.')
