...
import { Field, FieldDescription, FieldGroup } from '@aio-proxy/ui/components/field';
      ...
      <FieldGroup>
        <form.Field name="logging.retentionDays">
          {(field) => {
            ...
            return (
              <SettingsFieldRow
                label={m['dashboard.settings.retention_days']()}
                htmlFor={field.name}
        ...
        <form.Field name="logging.level">
          {(field) => (
            <SettingsFieldRow label={m['dashboard.settings.log_level']()} htmlFor={field.name}>
              <Select
                value={field.state.value}
                disabled={disabled}
                onValueChange={(value) => {
        ...
        <form.Field name="retryAfterCapSeconds">
          {(field) => {
            ...
            return (
              <SettingsFieldRow
                label={m['dashboard.settings.retry_after_cap']()}
                htmlFor={field.name}
                description={m['dashboard.settings.retry_description']()}
                error={invalid ? m['dashboard.settings.invalid']() : null}
              >
                <Input
                  id={field.name}
