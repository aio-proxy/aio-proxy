...
test('renders Provider health as a table without filter or column controls', () => {
  render(<ProviderHealthTable rows={rows} />);

  expect(screen.queryByRole('textbox', { name: /Filter providers|筛选提供商/u })).not.toBeInTheDocument();
  expect(screen.queryByRole('button', { name: /Provider columns|提供商列/u })).not.toBeInTheDocument();
