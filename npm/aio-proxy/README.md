...
Set `server.password` to protect the Dashboard. It does not protect model API endpoints; use `server.apiKeys` for those. The login is kept in the browser, so new tabs and browser restarts do not ask for the password again; visiting at least once every 7 days renews it automatically. Changing `server.password` immediately invalidates the login on every device.
