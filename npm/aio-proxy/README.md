...
The wizard defaults to keeping ChatGPT login available. In this mode it can use an existing proxy API Key; when no key exists, it skips key selection and uses the non-secret `aio-proxy-local` placeholder without creating a key or changing `server.apiKeys`. Native Codex login remains available. The command mode instead performs one AIO Proxy device approval during setup, stores the installation credential privately, and configures Codex to call `aiop agent auth codex --installation-id <uuid>`. The helper returns a short-lived access token and refreshes it silently when needed. Use `aiop agent revoke <installation-id>` to revoke command credentials.
