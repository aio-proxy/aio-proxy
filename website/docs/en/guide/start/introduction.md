...
3. Candidates are tried by descending Provider priority. Provider weight distributes traffic within the same priority. A Provider bound to an active session is preferred to preserve session continuity.
...
For example, when a client requests `gpt-5`, AIO Proxy selects from every Provider exposing `gpt-5` or its alias. Provider priority sets failover order; Provider weight then distributes traffic among Providers at the same priority.
